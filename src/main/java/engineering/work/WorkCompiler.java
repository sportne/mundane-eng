...
public final class WorkCompiler {
    ...
    public static Result compile(Path root,String manifest) {
        ...
        try {
            ...
            try {
                var m=map(Snapshots.json(input));keys(m,"format","files");if(!Versions.WORK_SET.equals(m.get("format")))throw new IllegalArgumentException("unsupported work selection format");
            ...
            for(String file:files) {
                try {
                    var source=reads.read(file,1024*1024);sources.add(object("path",file,"sha256",source.sha256()));var item=parse(source);
    ...
    private static Map<String,Object> parse(Snapshots.Snapshot snapshot) {
        ...
        try {
            ...
            keys(m,"format","status","dependencies","relations","planning");if(!Versions.WORK_SOURCE.equals(m.get("format")))throw new IllegalArgumentException("unsupported work source format");
