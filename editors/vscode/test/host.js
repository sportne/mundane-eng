...
async function run() {
  ...
  const software = enums.items.find(i => i.insertText === '"software"');
  assert.ok(software);
  const insertEnum = new vscode.WorkspaceEdit(); insertEnum.replace(child.uri, software.range, software.insertText);
  assert.ok(await vscode.workspace.applyEdit(insertEnum));
  assert.equal((await api.validate())[0].result.valid, true, 'accepted enum insertion is valid YAML and satisfies the declaration');
  ...
  await replace(schema, schemaText);
  await replace(child, attrText);
  const help = await vscode.commands.executeCommand('vscode.executeHoverProvider', child.uri, child.positionAt(child.getText().indexOf('discipline:') + 3));
  assert.equal(help.length, 1);
  assert.ok(help[0].contents[0].value.includes('required'));
  ...
  assert.ok(contents.value.includes('\\[run\\]'), 'description is literal Markdown text');
  await replace(schema, '{\n');
  assert.equal((await vscode.commands.executeCommand('vscode.executeHoverProvider', child.uri, child.positionAt(child.getText().indexOf('discipline:') + 3))).length, 0, 'invalid declaration clears hover');
  await replace(schema, schemaText);
  assert.equal((await vscode.commands.executeCommand('vscode.executeHoverProvider', child.uri, child.positionAt(child.getText().indexOf('storage shall') + 2))).length, 0);
  console.log('PASS actual hover provider: token ranges, types/values, unsaved literal documentation, invalidation and prose exclusion');
  ...
  await require('./engineering').run(api);
