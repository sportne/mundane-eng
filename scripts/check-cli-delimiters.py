...
with tempfile.TemporaryDirectory(prefix='literal-inputs-') as temporary:
    root = Path(temporary)
    (root / 'schema.json').write_bytes((ROOT / 'examples/attributes/requirement-attributes.json').read_bytes())
    ...
    for filename in ['--help', '--version', '--attribute-schema', '--']:
        ...
        for tool, main in TOOLS.items():
            ...
            args = ['--source=yaml-0.4', '--attribute-schema', 'schema.json'] + mode + ['--', filename]
            ...
            for command in commands:
                ...
                result = subprocess.run(command + ['--source=yaml-0.4', '--attribute-schema', 'schema.json', '--help'], cwd=root, capture_output=True, timeout=30)
