...
BASE=json.loads((ROOT/'examples/attributes/requirement-attributes.json').read_text())
...
with tempfile.TemporaryDirectory(prefix='attributes-') as folder:
    root=Path(folder);schema=root/'schema.json';source=root/'source.mreq.yaml'
    args=['--source=yaml-0.4','--attribute-schema','schema.json','source.mreq.yaml']
    ...
    run(root,args[:-1]+['--attribute-schema','schema.json',args[-1]],2)
    ...
    for invalid,code in [(original.replace('"type": "enum"','"type": "enum", "type": "enum"'),'attribute-schema-duplicate'),(original[:-1],'attribute-schema-invalid'),('\ufeff'+original,'attribute-schema-invalid'),(original.replace('"name":','// comment\n"name":'),'attribute-schema-invalid'),(original.replace('"name":','"x": "\\ud800", "name":'),'attribute-schema-invalid')]:schema.write_text(invalid);run(root,args,1,code)
    ...
    # Schema locations in SARIF are real JSON token points; unavailable has no region.
    reset();source.write_text(SOURCE.replace('"software"','"Software"'))
    sarif=['--source=yaml-0.4','--output=sarif','--root','.','--attribute-schema','schema.json','source.mreq.yaml']
    ...
    write(schema,BASE|{'name':'Bad'});out=json.loads(run(root,sarif,1,'attribute-schema-invalid').stdout);assert out['runs'][0]['results'][0]['locations'][0]['physicalLocation']['artifactLocation']['uri']=='schema.json'
    ...
    reset();os.symlink(schema,root/'alias.json');run(root,[x if x!='schema.json' else 'alias.json' for x in args],2,'attribute-schema-unavailable')
    with tempfile.TemporaryDirectory() as outside:
        p=Path(outside)/'schema.json';write(p,BASE);run(root,[x if x!='schema.json' else str(p) for x in sarif],2)
        os.symlink(p,root/'escape.json');run(root,[x if x!='schema.json' else 'escape.json' for x in sarif],2)
